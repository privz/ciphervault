...
## Google Chat compatibility

CipherVault 1.1.1 adds nested-frame selection support for Google Chat and Gmail-integrated Chat.

Quick Cipher now injects into all accessible frames and chooses the frame that actually owns the selected text. The extension also declares narrow host access for:
...
This is intentionally limited to Google Chat/Gmail compatibility; CipherVault still does not request a permanent `<all_urls>` permission. Code is only injected when Quick Cipher is explicitly triggered.
