...
async function runQuickCipher(commandTab) {
    ...
    try {
        // Google Chat can place the actual message/editor inside a nested frame.
        // Inject Quick Cipher into every frame that the extension is allowed to access.
        ...
        for (const frameId of frameIds) {
            try {
                ...
                // Some embedded frames may reject messaging; continue with the others.
            }
        }

        // Prefer a real user selection over a focused field fallback.
        // If more than one frame reports a selection, prefer the focused frame.
        ...
        const bubbleFrameId = capture?.frameId ?? 0;
        ...
        if (!credential.secret) {
            await showBubble(tab.id, bubbleFrameId, {
                status: "error",
...
async function showBubble(tabId, frameId, payload) {
